...
class MyLayer(Layer):
    ...
    def call(self, inputs):
        temp = inputs.get_shape()[1].value;
        if temp==None:
            temp=int(1)
        inputs_split = tf.split(inputs,temp,1)
        ...
        for i in range(1,len(inputs_split)):
            inputs_split[0] = inputs_split[0]+inputs_split[i]
        inputs_split[0]/=temp
        return inputs_split[0]  # remove dummy last dimension

def _bn_relu(layer, dropout=1, **params):
...
def add_output_layer(layer, **params):
    ...
    return Activation('softmax',name='outputs')(layer)
...
def build_network(**params):
    ...
    inputs_ecg = Input(shape=params['input_shape'],
                   dtype='float32',
                   name='ecg_inputs')
    inputs_pcg= Input(shape=params['input_shape'],
                     dtype='float32',
                     name='pcg_inputs')

    layer_ecg = add_resnet_layers(inputs_ecg, **params)
    layer_pcg = add_resnet_layers(inputs_pcg, **params)
    conca=concatenate([layer_ecg, layer_pcg], axis=-1)
    output = add_output_layer(conca, **params)
    #output = MyLayer()(output)
    model = Model(inputs=[inputs_ecg,inputs_pcg], outputs=[output])
